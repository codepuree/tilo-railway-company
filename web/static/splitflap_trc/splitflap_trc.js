...
function runStart() {
    ...
    const config = {
        ...
        "Train": {
            relW: 12.55, relH: 26, relTop: 33, relLeft: 21.25,
        ...
        "TrainNo": {
            relW: 9, relH: 26, relTop: 33, relLeft: 35.1,
            offsetTop: 6, charOffset: 15, fontSizeFactor: 110, 
            flapCharset: { 
                chars: [
                    " ", "001", "007", "012", "101", "123", "205", "211", "303", "372", "401",
                    "444", "500", "505", "612", "700", "711", "777", "800", "808", "901",
                    "923", "015", "118", "250", "333", "410", "555", "666", "810", "999", "5972"
                ], 
                options: {"default": ["weight-300"]}
            }
        },
    ...
    const paramCount = baseIds.length;
    ...
    baseIds.forEach((id, idx) => {
        if ($("#" + id).length && config[id]) {
            ...
            instances.push(ins);
            if (params[idx]) ins.display(decodeURIComponent(params[idx]));
        }
    });
    ...
    baseIds.forEach((id, idx) => {
        ...
        if ($("#" + id2).length && config[id]) {
            ...
            // 4. Parameter-Zuordnung (Index 8-15 in der URL)
            const paramIdx = idx + baseIds.length;
            if (params[paramIdx]) ins.display(decodeURIComponent(params[paramIdx]));
        }
    });
    ...
    // Clear localStorage before loading new parameters
    // for (let i = 0; i < ids.length; i++) {
    //     localStorage.removeItem('flap_index_' + ids[i]);
    // }

    setTimeout(() => {
        instances.forEach((ins, idx) => {
            if (params[idx]) {
                ins.display(decodeURIComponent(params[idx]));
            }
        });
    }, 500);
}
    ...
    if (Array.isArray(newParams)) {
        instances.forEach((ins, idx) => {
            if (newParams[idx] !== undefined) {
                ins.display(decodeURIComponent(newParams[idx]));
            }
        });
    }
}, false);